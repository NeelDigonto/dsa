...
template <typename T> std::ostream &operator<<(std::ostream &os, const std::vector<std::optional<T>> &v) {
  os << "[ ";
  for (auto it = v.cbegin(); it != v.cend(); ++it)
    if (it->has_value())
      os << it->value() << " ";
...
template <typename FwdIt> std::ostringstream printLinearDS(FwdIt _first, FwdIt _last) {
...
template <typename C> std::ostringstream print(const C &_c) { return printLinearDS(_c.begin(), _c.end()); }

template <typename Func, typename... Args> void benchmark(const std::string &_test_name, Func _func, Args &&..._args) {
...
template <typename T> void forEach(T _iterable, void (*_lambda)(const typename T::value_type &, std::size_t)) {
